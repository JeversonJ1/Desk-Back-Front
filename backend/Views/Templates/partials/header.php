...
<head>
  ...
  <style>
    :root {
      --bg-main: #0f0f0f;
      --bg-sidebar: linear-gradient(180deg, #000, #111);
      --bg-top: #0a0a0a;
      ...
      --glass-border: rgba(255, 216, 77, 0.1);
      --input-bg: #0f0f0f;
    }
    ...
    .w3-sidebar {
      ...
      overflow-y: auto;
      padding-top: 70px;
      /* Ajuste para melhor visual de perfil */
    ...
    .w3-sidebar a:hover:not(.active-link) {
      background-color: #ffcc0044 !important;
      /* Cor mais sutil no hover */
      color: #fff !important;
      ...
      <div class="w3-container w3-row">
        <div class="w3-container w3-center w3-padding">
          <div class="w3-container w3-center w3-padding">
            ...
            <img src="<?php echo htmlspecialchars($foto_exibir); ?>" class="w3-circle"
              style="width:120px; height:120px; object-fit: cover; border:2px solid #ffcc00;" alt="Foto de perfil"
              onerror="this.onerror=null; this.src='/img/logoperf.jpg';">
          </div>
          <h5 class="w3-margin-top">Bem-vindo, <strong><?= htmlspecialchars($session->get('usuario_nome')); ?></strong>
          </h5>
        </div>
        ...
        <div class="w3-bar-block">
          <?php if (in_array($session->get('usuario_tipo'), ['admin', 'vendedor'])): ?>
            <a href="/backend/admin/dashboard"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/admin/dashboard', $current_uri); ?>">
              <i class="fas fa-home fa-fw"></i> Início (Painel)
            </a>
            
            <?php if ($session->get('usuario_tipo') == 'admin'): ?>
              <a href="/backend/usuario/listar"
                class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/usuario/listar', $current_uri); ?>">
                <i class="fas fa-users fa-fw"></i> Usuários
              </a>
              <a href="/backend/produtos/listar"
                class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/produtos/listar', $current_uri); ?>">
                <i class="fas fa-tags fa-fw"></i> Produtos
              </a>
              <a href="/backend/itenspedidos/listar"
                class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/itenspedidos/listar', $current_uri); ?>">
                <i class="fab fa-dropbox fa-fw"></i> Itens Pedidos
              </a>
              <a href="/backend/avaliacao/listar"
                class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/avaliacao/listar', $current_uri); ?>">
                <i class="fas fa-star fa-fw"></i> Avaliações
              </a>
              <a href="/backend/admin/newsletter"
                class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/admin/newsletter', $current_uri); ?>">
                <i class="fas fa-envelope fa-fw"></i> Newsletter
              </a>
            <?php endif; ?>

            <a href="/backend/pedido/listar"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/pedido/listar', $current_uri); ?>">
              <i class="fas fa-shopping-cart fa-fw"></i> Todos Pedidos
            </a>
            <a href="/backend/cliente/listar"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/cliente/listar', $current_uri); ?>">
              <i class="fas fa-address-book fa-fw"></i> Clientes
            </a>
          <?php else: ?>
            <a href="/backend/cliente/dashboard"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/cliente/dashboard', $current_uri); ?>">
              <i class="fas fa-home fa-fw"></i> Início
            </a>
            <a href="/backend/cliente/avaliacoes"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/cliente/avaliacoes', $current_uri); ?>">
              <i class="fas fa-star fa-fw"></i> Avaliações
            </a>
          <?php endif; ?>
          ...
          <?php if ($session->get('usuario_tipo') != 'admin'): ?>
            <a href="/backend/cliente/meu-perfil/<?= htmlspecialchars($session->get('usuario_id') ?? '0') ?>"
              class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/cliente/meu-perfil', $current_uri); ?>">
          ...
          <a href="/backend/configuracoes"
            class="w3-bar-item w3-button w3-padding <?php echo isActive('/backend/configuracoes', $current_uri); ?>">
          ...
          <?php endif; ?>
          <br>
          <a href="/backend/logout" class="w3-bar-item w3-button w3-padding"
            style="background: linear-gradient(135deg, #ff6b6b 0%, #ff3b3b 100%); color: #000 !important; font-weight: 700; border-radius: 8px; margin-top: 10px; box-shadow: 0 4px 10px rgba(255, 59, 59, 0.25);">
